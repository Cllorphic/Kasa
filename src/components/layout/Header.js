 ...
 * Affiche les liens selon l'état de connexion et le rôle de l'utilisateur.
 * Inclut un menu burger pour mobile.
 */
export default function Header() {
  ...
  // Vérifie si l'utilisateur peut ajouter des propriétés
  ...
  return (
    <header className="w-full px-4 pt-4">
      <nav className="max-w-3xl mx-auto bg-white rounded-xl shadow-[0_4px_4px_rgba(182,182,182,0.05)] px-6 md:px-12 py-2 flex items-center justify-between relative">
        ...
        {/* Navigation gauche — liens principaux */}
        ...
        {/* Navigation droite — s'adapte selon l'état de connexion */}
        <div className="hidden md:flex items-center gap-5 shrink-0">
          {/* Rien tant que l'auth n'est pas prête pour éviter le flash */}
          {!isReady ? null : user ? (
            <>
              {/* Lien ajouter visible uniquement pour les owners/admins */}
              {isOwnerOrAdmin && (
                <Link
        ...
        {/* Menu mobile — s'ouvre au clic sur le burger */}
        {menuOpen && (
